...
garch_results <- data.frame(
  ...
  residuals = I(list())  # I() is used to allow lists in the dataframe
)
...
for (sector_name in names(sectors_log_returns)) {
  ...
  for (stock in stock_symbols) {
    ...
    # Extract the log-returns (close_lgret) for the current stock
    ...
    # Determine if the GARCH model was fitted (i.e., ARCH effects were detected)
    passed_garch_test <- !identical(log_returns, garch_result)
    
    # Add the results to the garch_results data frame
    garch_results <- rbind(
      garch_results,
      data.frame(
        sector = sector_name,
        ...
        residuals = I(list(garch_result))  # Store residuals (or original log-returns if no GARCH model was fitted)
      )
    )
  }
}

# Print the final garch_results data frame
print(garch_results)

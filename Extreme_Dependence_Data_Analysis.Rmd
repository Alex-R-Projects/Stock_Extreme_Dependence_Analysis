...
```{r}
# We are going to now build matrix plots to compare the dependence with in each stock. 

# The measures we are going to be using are MI (Mutual Information), Spearman Corelation, TDC (Tail-Dependent Coefficient at the 5% percentile)

# Helper function to reshape a matrix into a long format for ggplot
reshape_for_ggplot <- function(matrix, names) {
  as.data.frame(matrix) %>%
    rownames_to_column(var = "Var1") %>%
    pivot_longer(-Var1, names_to = "Var2", values_to = "value") %>%
    mutate(Var1 = factor(Var1, levels = names),
           Var2 = factor(Var2, levels = names)) %>%
    filter(as.numeric(Var1) > as.numeric(Var2))  # Keep only lower triangle
}

# Function to plot the Spearman Correlation
# Correlation Values will appear within each tile
plot_spearman_correlation_ggplot <- function(wide_data, title) {
  ...
  # Reshape for ggplot
  corr_data <- reshape_for_ggplot(spearman_corr, colnames(wide_data))
...
plot_spearman_correlation_ggplot(garch_communication_stable_wide, "Spearman Correlation for the Communication Services Sector During Stable Period")
plot_spearman_correlation_ggplot(garch_communication_pandemic_wide, "Spearman Correlation for the Communication Services Sector During Pandemic Period")


#####################################################

# Mutual Information Plots for the Communication Services
...
compute_mutual_information_matrix <- function(wide_data) {
  # Discretize each column of the data
...
# Helper function to reshape the MI matrix for ggplot2
reshape_for_ggplot <- function(matrix) {
  as.data.frame(matrix) %>%
    rownames_to_column(var = "Var1") %>%
    pivot_longer(-Var1, names_to = "Var2", values_to = "value") %>%
    mutate(Var1 = factor(Var1, levels = colnames(matrix)),
           Var2 = factor(Var2, levels = colnames(matrix))) %>%
    filter(as.numeric(Var1) > as.numeric(Var2))  # Keep only lower triangle
}

# Plotting function for Mutual Information matrix
plot_mutual_information_matrix <- function(wide_data, title) {
  # Step 1: Compute the mutual information matrix
  mi_matrix <- compute_mutual_information_matrix(wide_data)
  
  # Step 2: Reshape for ggplot2
  mi_data <- reshape_for_ggplot(mi_matrix)
  
  # Step 3: Create the plot
  ggplot(mi_data, aes(x = Var1, y = Var2, fill = value)) +
    geom_tile(color = "white") +
    geom_text(aes(label = round(value, 2)), color = "black", size = 3) +  # Display MI values
    scale_fill_gradient(low = "white", high = "darkgreen", name = "Mutual\nInformation") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(title = title, x = "", y = "")
}

plot_mutual_information_matrix(garch_communication_stable_wide, "Mutual Information for Communication Services Sector During Stable Period")
plot_mutual_information_matrix(garch_communication_pandemic_wide, "Mutual Information for Communication Services Sector During Pandemic Period ")

#############################################################


# Function to compute the Tail Dependence Coefficient (TDC) matrix for the lower tail
compute_tdc_matrix <- function(data, percentile = 0.05) {
  n <- nrow(data)
  percentile_value <- quantile(data[[1]], probs = percentile)
  ...
  # Compute the TDC matrix for the lower tail using Empirical Tail Dependence Coefficient (EmpTC)
  tdc_mat <- FRAPO::tdc(data, method = "EmpTC", lower = TRUE, k = k)
...
plot_tdc_mat <- function(data, cor_levels, percentile = 0.05) {
  
  # Compute the TDC matrix
  tdc_mat <- compute_tdc_matrix(data, percentile = percentile)
    
  # Set upper triangle to NA to display only the lower triangle
  tdc_mat[upper.tri(tdc_mat, diag = TRUE)] <- NA
  
  # Convert the matrix to long format for ggplot2
  tdc_long <- reshape2::melt(tdc_mat, na.rm = TRUE) %>% 
    mutate(
      Var1 = factor(Var1, levels = cor_levels, ordered = TRUE),
      Var2 = factor(Var2, levels = cor_levels, ordered = TRUE)
    ) %>%
    filter(!is.na(value))  # Remove NA values for cleaner plot
  
  # Plotting the TDC matrix using ggplot2
  tdc_plot <- ggplot(
    tdc_long, 
    aes(
      x = Var1, y = Var2, 
    ...
    geom_tile(color = "white") + 
    geom_text(color = "black", size = 3, na.rm = TRUE) +
    scale_x_discrete(expand = c(0,0)) +
    scale_y_discrete(limits = rev(levels(tdc_long$Var2)), expand = c(0,0)) +
    scale_fill_distiller(
      palette = "RdBu", 
      direction = 1, 
      limits = c(0, 1), 
    ...
    labs(title = "Tail Dependence Coefficient (TDC) Matrix") +
    theme_minimal() +
    theme(
      axis.title = element_blank(),
      axis.text.x = element_text(
...
# Example for the communication stable GARCH data frame
cor_levels_communication <- colnames(garch_communication_stable_wide)
plot_tdc_mat(garch_communication_stable_wide, cor_levels = cor_levels_communication, percentile = 0.05)

# Example for the communication stable GARCH data frame
cor_levels_communication <- colnames(garch_communication_pandemic_wide)
plot_tdc_mat(garch_communication_pandemic_wide, cor_levels = cor_levels_communication, percentile = 0.05)

...
plot_mutual_information_matrix(garch_communication_stable_wide, "Mutual Information for the Communication Services Sector (Stable Period)", communication_tickers)
plot_mutual_information_matrix(garch_communication_pandemic_wide, "Mutual Information for the Communication Services Sector (Pandemic Period)", communication_tickers)

tdc_stable_comm <-plot_tdc_mat(garch_communication_stable_wide, "TDC for theCommunication Services Sector (Stable Period)",tickers =  communication_tickers, percentile = 0.05)
...
mutualinfo_stable_dis <-plot_mutual_information_matrix(garch_discretionary_stable_wide, "Mutual Information for the Consumer Discretionary Sector (Stable Period)", discretionary_tickers)
mutualinfo_pan_dis <-plot_mutual_information_matrix(garch_discretionary_pandemic_wide, "Mutual Information for the Consumer Discretionary Sector (Pandemic Period)", discretionary_tickers)
...
plot_mutual_information_matrix(garch_staples_stable_wide, "Mutual Information for the Consumer Staples Sector (Stable Period)", staples_tickers)
plot_mutual_information_matrix(garch_staples_pandemic_wide, "Mutual Information for the Consumer Staples Sector (Pandemic Period)", staples_tickers)
...
```{r}
# Helper function to reshape a for a full matrix
reshape_for_ggplot_all_sector <- function(matrix, names) {
  as.data.frame(matrix) %>%
    rownames_to_column(var = "Var1") %>%
    pivot_longer(-Var1, names_to = "Var2", values_to = "value") %>%
    mutate(
...
# Function to plot Spearman Correlation (Full Matrix)
plot_spearman_correlation_ggplot_all_sector <- function(wide_data, title, tickers, show_lower_triangle = FALSE) {
  # Calculate Spearman correlation matrix
  spearman_corr <- cor(wide_data, method = "spearman", use = "pairwise.complete.obs")
  
  # Reshape for ggplot
  corr_data <- reshape_for_ggplot_all_sector(spearman_corr, tickers)
...
# Function to plot Tail Dependence Coefficient (Full Matrix)
plot_tdc_mat_all_sector <- function(data, title, tickers = NULL, percentile = 0.05,
                                      families = NA, rotations = TRUE) {
  # Compute the TDC matrix using the vineCopula approach.
  tdc_mat <- compute_tdc_matrix(data, percentile = percentile,
                                families = families, rotations = rotations)
  
  # For a full matrix plot (including the main diagonal and upper triangle),
  # simply comment out or remove the line that sets the upper triangle to NA.

...
plot_tdc_mat <- function(data, title, tickers, percentile = 0.05) {
 ...
 # tdc_mat[upper.tri(tdc_mat)] <- NA                                                  # Delete when you want lower triangle
...
plot_spearman_correlation_ggplot2 <- function(wide_data, title, tickers, show_lower_triangle = FALSE, show_weak_corr = TRUE) {
  # Calculate Spearman correlation matrix
  spearman_corr <- cor(wide_data, method = "spearman", use = "pairwise.complete.obs")
  ...
  # Optionally filter for the lower triangle only
  if (show_lower_triangle) {
    corr_data <- corr_data %>%
      filter(as.numeric(Var1) > as.numeric(Var2))
  }
  
  # Optionally hide weak correlations
  if (!show_weak_corr) {
    corr_data <- corr_data %>%
      mutate(label = ifelse(abs(value) > 0.2, sprintf("%.2f", value), ""))  # Only show stronger correlations
  } else {
    corr_data <- corr_data %>%
      mutate(label = sprintf("%.2f", value))  # Show all correlation values
  }
  
  # Plot with reduced title and legend size
  ggplot(corr_data, aes(x = Var1, y = Var2, fill = value)) +
    ...
    theme_classic() +
    theme(
      axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 6, face = "bold"),  # Axis label size
      axis.text.y = element_text(size = 6, face = "bold"),
      axis.title = element_blank(),
      panel.grid = element_blank(),
      plot.title = element_text(size = 10, face = "bold", hjust = 0.5),  # Smaller title
      legend.title = element_text(size = 10),  # Smaller legend title
      legend.text = element_text(size = 6),  # Smaller legend labels
      legend.key.height = unit(0.3, "cm"),  # Reduce legend key size
...
test <-plot_spearman_correlation_ggplot2(all_sector_wide_stable, "Spearman Correlation Across All Sectors (Stable Period)", all_tickers)
test2 <-plot_spearman_correlation_ggplot2(all_sector_wide_pandemic, "Spearman Correlation for Across All Sectors (Pandemic Period)", all_tickers)
...
test5 <-plot_tdc_mat(all_sector_wide_stable, "TDC Across All Sectors (Stable Period)", all_tickers)
test6 <-plot_tdc_mat(all_sector_wide_pandemic, "TDC Across All Sectors (Pandemic Period)", all_tickers)
...
ggsave("test.png", plot = test, width = 5120, height = 2880,units = 'px',  dpi = 400) # Saves Spearman Correlation (stable period)

ggsave("test2.png", plot = test2, width = 5120, height = 2880,units = 'px',  dpi = 400) # Saves Spearman Correlation (pandemic period)


ggsave("test5.png", plot = test5, width = 5120, height = 2880,units = 'px',  dpi = 400) # Saves Spearman Correlation (stable period)

ggsave("test6.png", plot = test6, width = 5120, height = 2880,units = 'px',  dpi = 400) # Saves Spearman Correlation (pandemic period)
